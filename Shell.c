  ...
  if (interactive_mode)
    {
      ...
      snprintf(buffer, 1024, "%s:~%s$ ",hostname,getcwd(NULL, pathconf(".", _PC_PATH_MAX)));
...
main (int argc, char **argv) 
{
  ...
  while (1){
    if (my_yyparse () == 0) {  /* L'analyse a abouti */   
	  ...
	  fflush(stdout);
      status=evaluer_expr(ExpressionAnalysee,0,1,2,0);
      expression_free(ExpressionAnalysee);
      while(waitpid(-1 , &status_zombie , WNOHANG)>=0);//élimination des Zombies 
